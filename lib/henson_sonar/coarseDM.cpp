...
// Eigen::Matrix<int, 169, 1681> dictionaryMatrix(int pixelX, int pixelY,
// cv::Mat imgTarget, cv::Mat imgRef) {
Eigen::Matrix<float, Dynamic, Dynamic> CoarseDM::dictionaryMatrix(
...
// void getTargetErrorOMP(const Matrix<float, Dynamic, Dynamic>& dictA, const
// VectorXf& targetY, VectorXf& xHatOut, VectorXf& errorOut) {
int CoarseDM::getTargetErrorOMP(const Matrix<float, Dynamic, Dynamic>& dictA,
  ...
  Eigen::VectorXf error(
      dictA.rows());  // make error vector, duplicate values in targetY
  ...
  while (true) {
    ...
    // ROS_INFO_STREAM("Selected index " << maxIndex << " with value of " <<
    // maxValue);
    ...
    Eigen::MatrixXf dictASupport(
        dictA.rows(),
        supports.size());  // Matrix of A ONLY containing the support
    int idx = 0;
    ...
    // Find least-squares
    Eigen::MatrixXf xTemp =
        dictASupport.bdcSvd(ComputeThinU | ComputeThinV).solve(targetY);
    // cout << "xTemp: \n" << xTemp << "\n";
    ...
    ROS_DEBUG_STREAM("Error norm " << errorNorm << " for iteration "
                                   << iteration);
    ...
    if (errorNorm < OMP_ERROR_THRESH) {
      // ROS_INFO_STREAM("Found solution with error norm of " << errorNorm << "
      // which is below the threshold of " << OMP_ERROR_THRESH);
      ...
      // ROS_WARN_STREAM("Maximium iterations of " << OMP_ITERATION_MAX << "
      // exceeded. Breaking");
  ...
  // cout << "final xhat: \n" << xHat << "\n";
  // cout << "result is " << result.cols() << " columns by " << result.rows() <<
  // "rows \n";
...
std::vector<std::vector<int>> CoarseDM::getSamplePoints(const cv::Mat& img) {
  ...
  while (num_samples < OMP_NUM_SAMPLES) {
    // TODO: determine whether to generate x and y OR use hilbert space filling
    // curve
    int border_size = 41;
    ...
    // to make lower intensity points less likely to be saved, just increase the
    // third number in the call to rangeMap int mod_val = rangeMap(curVal, 0,
    // 1680, 1, 11); mod_val = flipRange(mod_val, 1, 11); cout << mod_val <<
    // endl; int random = rand() % mod_val; cout << "val: " << curVal; cout <<
    // random << endl; if (random == 0) save = true;
    ...
    // std::vector<int> sample_point = {curX, curY};
    // if (std::find(result.begin(), result.end(), sample_point) !=
    // result.end()) {
    //   result.push_back(sample_point);
    //   num_samples++;
    // }
...
// void CoarseDM::interpolateOMPimage(const cv::Mat& img, cv::Mat& out, int x,
// int y) { input:
...
int CoarseDM::interpolateOMPimage(
    const cv::Mat& img, cv::Mat& out, int x, int y,
    const std::unordered_set<std::vector<int>, VectorHash>& sample_points_set) {
  ...
  for (int j = y - 6; j <= y + 6; j++) {
    for (int i = x - 6; i <= x + 6; i++) {
      ...
      bool valid = false;
      if (sample_points_set.find(cur_point) != sample_points_set.end())
        valid = true;
  ...
  // find bin with most points, set all points in 13x13 window to avg value
  // of
  // bin
  int max_bin_index = 0;
...
int CoarseDM::rangeMap(int in, int in_start, int in_end, int out_start,
                       int out_end) {
...
int CoarseDM::flipRange(int in, int min, int max) { return (max + min) - in; }
...
void CoarseDM::compareDM(cv::Mat& forwardDM, const cv::Mat& backwardDM) {
  // comparison for every pixel position ξ ∈ X (every pixel in image? or sample
  // points?) doing all pixels for now

  cv::Mat valid_estimates =
      Mat(forwardDM.rows, forwardDM.cols, CV_8UC1, Scalar(0));
  ...
  for (int i = 0; i < forwardDM.cols; i++) {
    for (int j = 0; j < forwardDM.rows; j++) {
      ...
      bool valid = ((abs(bwd_x_shift + fwd_x_shift) +
                     abs(bwd_y_shift + fwd_y_shift)) > 2);
      if (valid)
        valid_points.push_back({i, j});
      else
        invalid_points.push_back({i, j});
  ...
  for (Point& p : invalid_points) {
    ...
    // NOTE: extremely inefficient. need to implement spiral search or something
    // better
    int dist, min_idx;
    ...
    for (int i = 1; i < valid_points.size(); i++) {
      int cur =
          pow(valid_points[i].x - p.x, 2) + pow(valid_points[i].y - p.y, 2);
      if (cur < dist) {
        cur = dist;
        min_idx = i;
      }

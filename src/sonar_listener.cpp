...
void threadedOMP(const std::vector<std::vector<int>>& points,
                 const cv::Mat& curCartesian, const cv::Mat& prevCartesian,
                 cv::Mat& omp_collage) {
  for (const std::vector<int>& point : points) {
    ...
    Eigen::Matrix<float, Dynamic, Dynamic> dict_matrix =
        coarse_dm.dictionaryMatrix(curX, curY, prevCartesian);
    ...
    // myfile.open("OMP_image_value_after_write_(in_thread).txt",
    // std::ios::app); myfile << "image " << std::to_string(save_frame_num) << "
    // " << curX << ", "<< curY << " = " << omp_collage.at<int>(curY, curX) <<
    // endl; myfile.close();
    ...
    //       myfile << "image " << std::to_string(save_frame_num) << " " << i <<
    //       ", "<< j << " = " << omp_collage.at<int>(j,i) << endl;
    //     }
    //   }
    // }
    // myfile.close();
...
cv::Mat getCoarseDM(const cv::Mat& curCartesian, const cv::Mat& prevCartesian) {
  // save original image
  std::string filename =
      "/home/tanner/catkin_ws/src/henson_sonar/output/frame" +
      std::to_string(save_frame_num) + ".png";
  ...
  std::vector<std::vector<int>> sample_points =
      coarse_dm.getSamplePoints(curCartesian);
  ...
  // make OMP image
  cv::Mat omp_collage =
      Mat(curCartesian.rows, curCartesian.cols, CV_32SC1, Scalar(0));
  start = std::chrono::system_clock::now();
  std::thread threads[OMP_THREADS];
  for (int i = 0; i < OMP_THREADS; i++) {
    std::vector<std::vector<int>> thread_points = thread_jobs[i];
    threads[i] = std::thread(threadedOMP, thread_points, std::ref(curCartesian),
                             std::ref(prevCartesian), std::ref(omp_collage));
  }
  ...
  filename = "/home/tanner/catkin_ws/src/henson_sonar/output/omp" +
             std::to_string(save_frame_num) + ".png";
  ...
  start = std::chrono::system_clock::now();
  cv::Mat interpolation_img =
      Mat(curCartesian.rows, curCartesian.cols, CV_32SC1, Scalar(0));

  std::unordered_set<std::vector<int>, VectorHash> sample_points_set;
  std::copy(sample_points.begin(), sample_points.end(),
            std::inserter(sample_points_set, sample_points_set.end()));
  for (std::vector<int>& point : sample_points) {
    int x = point[0];
    int y = point[1];
    // cout << "current point: " << x << ", " << y << endl;
    int val = coarse_dm.interpolateOMPimage(omp_collage, interpolation_img, x,
                                            y, sample_points_set);
...
void sonarCallback(const imaging_sonar_msgs::SonarImage::ConstPtr& msg) {
  ...
  // cv::waitKey(1);
  cv::Mat curCartesian;
  cv::copyMakeBorder(float_cartesian, curCartesian, BORDER_WIDTH, BORDER_WIDTH,
                     BORDER_WIDTH, BORDER_WIDTH, BORDER_CONSTANT, 0.0);
  int img_q_size = 5;
  ...
  if (frame_num > 80) {
    ...
    if (img_queue.size() >= img_q_size) {
      cout << "image queue filled, processing " << img_queue.size()
           << " images\n";
      while (img_queue.size() != 0) {
        ...
        if (first) {
          ...
          cv::Mat save;
          std::string filename =
              "/home/tanner/catkin_ws/src/henson_sonar/output/fwd_interp" +
              std::to_string(save_frame_num) + ".png";
          ...
          filename =
              "/home/tanner/catkin_ws/src/henson_sonar/output/bwd_interp" +
              std::to_string(save_frame_num) + ".png";
          ...
          elapsed_time = end - start;
          cout << "coarse DM comparison complete in: " << elapsed_time.count()
               << "s\n";

          // save final forward coarse DM
          filename =
              "/home/tanner/catkin_ws/src/henson_sonar/output/final_coarseDM" +
              std::to_string(save_frame_num) + ".png";
          save = coarse_dm.scaleImg(fwdDM);
          cv::imwrite(filename, save);
        }
...
int main(int argc, char** argv) {
  ...
  // ros::Subscriber sub = n.subscribe("/oculus/sonar_image", 1000,
  // photoCallback);
